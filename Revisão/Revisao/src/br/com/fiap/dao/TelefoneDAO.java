...
public class TelefoneDAO{
	...
	public Telefone buscar(String departamento, Connection conexao) throws Exception{
		 ...
		 * como a tabela é pequena, usei '*', mas nao é o aconselhavel, como vcs sabem.
		 */
		PreparedStatement estrutura = conexao.prepareStatement("SELECT * FROM TB_TELEFONE WHERE (DEPARTAMENTO) = ?");
		
		/*
		 * Aqui fizemos a comparação da busca já no comando sql, então,
		 * o metodo busca o telefone, a partir do departamento recebido nos paramentos do metodo
		 */
		estrutura.setString(1, departamento);
		
		/*Agora precisamos criar uma variavel que nos ajudará a receber os valores do banco
		 * e coloca-los no objeto telefone (que instanciamos lá no inicio da classe) e
		 * poderemos retornar esse objeto já com o valor daquilo que o usuario busca
		 */
		...
		//o if é para ele buscar em todos os dados gravados no banco
		if(resultadoBusca.next()){
			telefone.setDdd(resultadoBusca.getInt("DDD"));
